...
// @Success 200 {object} code.Response "{"code":200,"data":[]theme.ResponseTheme,"msg":"ok"}"
...
// @Success 200 {object} code.Response "{"code":200,"data":theme.Theme,"msg":"ok"}"
// @Router /theme/delete [delete]
func (*themeApi) Delete(c *gin.Context) {
	id := c.Query("id")
	if id == "" {
		code2.FailResponse(code2.ErrorMissingId, c)
		return
	}
...
func (*themeApi) Update(c *gin.Context) {
	...
	var t theme.Theme
	if err := global.DB.Where("id=?", themeRequest.ID).First(&t).Error; err != nil {
		code2.FailResponse(code2.ErrorNotFound, c)
		return
	}
	if t.UserID != userID {
		code2.FailResponse(code2.ErrorNotFound, c)
