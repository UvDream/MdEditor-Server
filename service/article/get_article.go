...
func (*ToArticleService) GetArticleListService(query article.ListArticleRequest, uuid string, c *gin.Context) (list *[]article.Article, total int64, msg string, err error) {
	...
	//查询总数
	if err = db.Where("user_id = ?", uuid).Count(&total).Error; err != nil {
		return nil, 0, "查询总数失败", err
	}
	//查询列表
	if err = db.Where("user_id = ?", uuid).Preload("Tags").Preload("Categories").Preload("User").Order("create_time desc").Scopes(utils.Paginator(c)).Find(&articleList).Error; err != nil {
...
func (*ToArticleService) GetArticleDetailService(id string) (list article.Article, msg string, err error) {
	err = global.DB.Where("id=?", id).Preload("Tags").Preload("Categories").Preload("User").First(&list).Error
	if err != nil {
