...
func (*SysUserService) Login(username string, password string) (user system.User, token string, msg string, err error) {
	//先判定数据库是否存在
	if global.DB == nil {
		return user, "", "数据库不存在", fmt.Errorf("数据库未初始化")
	}
	if err := global.DB.Where("user_name = ?", username).First(&user).Error; err != nil {
		return user, "", "查找用户信息错误", err
	} else {
		if ok := utils.BcryptCheck(password, user.Password); !ok {
			return user, "", "密码错误", fmt.Errorf("密码错误")
		} else {
			//	颁发token
			j := &utils.JWT{
