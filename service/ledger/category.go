...
func (*LedgersService) GetLedgerCategoryList(id string) ([]ledger.CategoryLedger, int, error) {
	db := global.DB
	//查询账本
	var le ledger.Ledger
	if err := db.Model(&ledger.Ledger{}).Preload(clause.Associations).Where("id = ?", id).First(&le).Error; err != nil {
